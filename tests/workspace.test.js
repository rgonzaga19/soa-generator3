...
for(const page of ['index','batch']) test(`${page} workspace navigation and theme controls`,async()=>{
 ...
 function element(dataset={}){return {dataset,style:{},attributes:{},classList:{add(){},remove(){}},innerHTML:'',value:'',textContent:'',children:[],setAttribute(k,v){this.attributes[k]=v},getAttribute(k){return this.attributes[k]},removeAttribute(k){delete this.attributes[k]},addEventListener(name,fn){this[name]=fn},appendChild(n){this.children.push(n)}};}
 for(const m of html.matchAll(/id="([^"]+)"/g)){assert.ok(!nodes.has(m[1]),`Duplicate ID ${m[1]}`);nodes.set(m[1],element());}
 const choices=['light','dark'].map(themeChoice=>element({themeChoice}));const nav=['individual','batch','settings','about'].map(view=>element({view}));const container=element();const root=element();root.setAttribute('data-theme','light');const errors=[];
 const context=vm.createContext({document:{getElementById:id=>nodes.get(id)||null,querySelector:()=>container,querySelectorAll:q=>q==='[data-theme-choice]'?choices:nav,createElement:element,documentElement:root},window:{scrollTo(){},electronAPI:{getAppVersion:async()=> '2.0.5',getLicenseKey:async()=> ''}},localStorage:{setItem(){}},console:{log(){},error:e=>errors.push(String(e))},setTimeout,clearTimeout,AbortController,fetch:async()=>{throw Error('Unexpected fetch')}});
